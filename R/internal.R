...
getVFT <- function(mesh, beforeCheck = FALSE) {
  ...
  if(inherits(mesh, "mesh3d")) {
    ...
    rmesh <- checkMesh(mesh[["vertices"]], mesh[["faces"]], aslist = TRUE)
...
fromR <- function(mesh) {
  ...
  faces    <- faces <- lapply(seq_len(nrow(mesh$faces)), function(i) { mesh$faces[i, ] - 1L })
